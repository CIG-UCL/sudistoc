...
class sudistocSingle_net(modelio.LoadableModel):
    """
    sudistoc network for suceptibility distortion correction.
    """
    ...
    def __init__(self,
        ...
        # ensure compatible settings
        if not transfo_sup and not image_sup:
            raise ValueError('At least one of transfo_sup, image_sup or unsup has to be True')
    ...
    def register(self):
        ...
        return tf.keras.Model(self.inputs, [self.references.y_source, self.references.pos_flow], training=False)
